...
function buildChartOption() {
  ...
  // Y 轴范围自适应：取所有曲线最大绝对值，向上取整到步长（对称坐标）
  let maxAbs = 0;
  items.forEach((it) => it.values.forEach((v) => {
    if (v != null) { const a = Math.abs(v); if (a > maxAbs) maxAbs = a; }
  }));
  const stepYi = chartState.type === 'stock' ? 5 : 50; // 个股 5 亿步长，行业/概念 50 亿步长
  const defaultRange = { industry: 150, concept: 300, stock: 35 }[chartState.type] || 150; // 各类型默认坐标
  const rangeYi = Math.max(defaultRange, Math.ceil((maxAbs / 1e8) / stepYi) * stepYi);
